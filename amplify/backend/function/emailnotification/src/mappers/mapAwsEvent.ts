...
export default function mapAwsEvent(
  ...
  if (event.missingAccessories) {
    ...
    missingAccessories = models.map((model) => {
      ...
      return {
        ...
        lastReturnedBy: getString(missing.lastReturnedBy, 'lastReturnedBy'),
    ...
    advertPickUps = pickUps
      ...
      .map((pickUp) => {
        return {
          ...
          reservedBySub: getString(pickUp.reservedBySub, 'pickUpReservedBySub'),
        } as AdvertPickUp;
      });
  }
  ...
  if (event.advertBorrowCalendar) {
    ...
    advertBorrowCalendar = {
      ...
      calendarEvents: getList(calendar.calendarEvents, 'calendarEvents').map(
        (calendarEventModel) => {
          ...
          const returnDateTime = calendarEvent.returnDateTime
            ...
            : null;
          const quantity = getNumber(calendarEvent.quantity, 'eventQuantity');
