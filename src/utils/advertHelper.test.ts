...
const user2 = {
    ...user1,
    sub: '0b293fb6-3cad-42a4-a476-9e115406f411',
    company: 'Foobar',
};
...
const user5 = {
    ...user1,
    sub: '0b293fb6-3cad-42a4-a476-9e115406f400',
    company: 'Foobar',
};
...
const recycleAdvert = {
    ...RecycleAdvert,
    material: [] as ItemAMaterial[],
    areaOfUse: [] as ItemAreaOfUse[],
...
describe('Get status', () => {
    test('Recycle advert returns available', () =>
        expect(getStatus(recycleAdvert as Advert, user1, date)).toBe(
            'available',
        ));
    ...
    test('Borrow advert returns borrowPermissionDenied', () =>
        expect(getStatus(borrowAdvert1, user5, date)).toBe(
            'borrowPermissionDenied',
        ));
});
...
describe('Checks if user has permission to borrow advert', () => {
    test('Returns true if advert has no restrictions', () =>
        expect(
            hasUserBorrowPermission(user1, BorrowAdvert as Advert),
        ).toBeTruthy());
